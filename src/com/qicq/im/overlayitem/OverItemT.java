...
public class OverItemT extends ItemizedOverlay<AbstaractOverlayItem> {
	...
	public OverItemT( Context context, LBSApp app,MapView mMapView, View mPopView) {
		super(null);
	...
	protected boolean onTap(int i) {
		...
		TextView info = (TextView) mMapView.findViewById(R.id.popview_info);
		Button button = (Button) mMapView.findViewById(R.id.popButton);

		if(mMapView.getZoomLevel() > 11){
			Log.v(">11","clicked");
			user = app.getUser(((PeopleOverlayItem)mGeoList.get(i)).uid);
			if(user.demand != null)
				info.setText(user.age + " " + user.sex + " " + user.demand.name);
			else
				info.setText(user.age + " " + user.sex);
