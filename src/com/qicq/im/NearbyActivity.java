...
public class NearbyActivity extends MapActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * mSearch = new MKSearch(); mSearch.init(mBMapMan, new
		 * MKSearchListener(){
		 * 
		 * public void onGetAddrResult(MKAddrInfo res, int errno) { if(errno !=
		 * 0){ Log.v("MKSearch","get add error!" + errno); }
		 * 
		 ...
		 * res.addressComponents.district + res.addressComponents.street +
		 * res.addressComponents.streetNumber);
		 * //LBSToast.makeText(NearbyActivity.this,
		 * strInfo,Toast.LENGTH_LONG).show(); Log.v("MKSearch",strInfo);
		 * Log.v("MKSearch strAddr",res.strAddr);
		 * Log.v("MKSearch strBusiness",res.strBusiness); }
		 * 
		 * public void onGetBusDetailResult(MKBusLineResult arg0, int arg1) { }
		 * 
		 * public void onGetDrivingRouteResult(MKDrivingRouteResult arg0, int
		 * arg1) { }
		 * 
		 ...
		 * { }
		 * 
		 * public void onGetTransitRouteResult(MKTransitRouteResult arg0, int
		 * arg1) { }
		 * 
		 * public void onGetWalkingRouteResult(MKWalkingRouteResult arg0, int
		 * arg1) { }
		 * 
		 * });
		 */
	...
	private LocationListener mLocationListener = new LocationListener() {
		...
		public void onLocationChanged(Location location) {
			if (location != null) {
				...
				String strLog = String.format("Lng:%f Lat:%f",
	...
	private void initView() {
		...
		// 创建点击mark时的弹出泡泡
		mPopView = super.getLayoutInflater().inflate(R.layout.popview, null);//修改此处来更改按泡泡时弹出的东东，然后在弹出的东东上点击的时候再到达新的activity
		mMapView.addView(mPopView, new MapView.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, null,
				MapView.LayoutParams.TOP_LEFT));
		mPopView.setVisibility(View.GONE);
		//mPopView.setBackgroundResource();添加背景
	}
	...
	private void updateOverlayItems(int gender, int ageLevel, String updatetime) {
		if (mMapView.getZoomLevel() <= 11) {
			...
			List<LocationCluster> clusters = app.GetLocationCluster(
					p1.getLatitudeE6(), p1.getLongitudeE6(),
					p2.getLatitudeE6(), p2.getLongitudeE6(), gender, ageLevel,
					updatetime);
